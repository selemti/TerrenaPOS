...
// Configuración de impresora (ajustar según SO)
$printer_device = PHP_OS === 'WINNT' ? 'EPSON_TM_T20II' : '/dev/usb/lp0'; // Cambiar según nombre de impresora o puerto USB
$auto_print_pdf = true; // Configurar para impresión automática del PDF

// Función para imprimir en impresora USB
function printPrecorteThermal($precorte_id, $terminal_id, $cashier_user_id, $from_ts, $to_ts, $opening_cash, $system, $counted_cash, $declared_credit, $declared_debit, $declared_custom, $notes, $tarjetas, $customs, $conn) {
    try {
        $connector = PHP_OS === 'WINNT' 
            ? new WindowsPrintConnector("EPSON_TM_T20II") // Nombre de impresora en Windows
            : new FilePrintConnector("/dev/usb/lp0"); // Puerto USB en Linux
...
// Función para generar y manejar PDF
function generatePrecortePDF($precorte_id, $terminal_id, $cashier_user_id, $from_ts, $to_ts, $opening_cash, $system, $counted_cash, $declared_credit, $declared_debit, $declared_custom, $notes, $tarjetas, $customs, $action = 'download', $auto_print = false) {
    $dompdf = new Dompdf();
    $dompdf->setPaper('B7', 'portrait'); // Aproximado para tiras térmicas
    ob_start();
    ?>
    <style>
    ...
    if ($action === 'download') {
        ...
        if (PHP_OS === 'WINNT') {
            exec("print /d:\\\\.\\EPSON_TM_T20II \"$pdf_path\""); // Ajustar nombre de impresora
        } else {
            exec("lp -d EPSON_TM_T20II \"$pdf_path\""); // Ajustar nombre de impresora en Linux
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'charge') {
        ...
        $card_type = $_POST['card_type'] ?? ''; // Nuevo: marca de tarjeta
        $card_reader = $_POST['card_reader'] ?? ''; // Nuevo: terminal bancaria
    ...
    if ($action === 'save_precorte') {
        ...
        if ($voided_count > 5 || $total_discounts > 100) {
            if ($user_type !== 'SUPERVISOR') {
                die(json_encode(['error' => "Umbrales excedidos: $voided_count anulaciones (>5), $total_discounts MXN en descuentos (>$100)"]));
            }
        }
        ...
        $result = pg_query_params($conn, "
            ...
            SELECT $1, t.location, $2, $3, $4, $5, sys_cash, sys_credit, sys_debit, sys_custom_total,
                   sys_payouts, $6, $7, $8, $9, 'SUBMITTED', $2, $10
        ...
        if ($result) {
            $precorte_id = pg_fetch_result($result, 0, 'id');
            foreach ($denoms as $denom => $qty) {
                $subtotal = $denom * $qty;
                if ($subtotal > 0) {
                    pg_query_params($conn, "
            ...
            // Imprimir en térmica
            $print_thermal_result = printPrecorteThermal($precorte_id, $terminal_id, $cashier_user_id, $from_ts, $to_ts, $opening_cash, $system, $counted_cash, $declared_credit, $declared_debit, $declared_custom, $notes, $tarjetas, $customs, $conn);

            // Generar PDF (para impresión automática)
            if ($auto_print_pdf) {
                $print_pdf_result = generatePrecortePDF($precorte_id, $terminal_id, $cashier_user_id, $from_ts, $to_ts, $opening_cash, $system, $counted_cash, $declared_credit, $declared_debit, $declared_custom, $notes, $tarjetas, $customs, 'print', true);
            } else {
    ...
    if ($action === 'download_pdf') {
        ...
        $precorte_data = pg_query_params($conn, "
            SELECT opening_cash, counted_cash, declared_credit, declared_debit, declared_custom, notes
            FROM public.pc_precorte
            WHERE id = $1",
            [$precorte_id]);
        $precorte = pg_fetch_assoc($precorte_data);
        generatePrecortePDF($precorte_id, $terminal_id, $cashier_user_id, $from_ts, $to_ts, $precorte['opening_cash'], $system, $precorte['counted_cash'], $precorte['declared_credit'], $precorte['declared_debit'], $precorte['declared_custom'], $precorte['notes'], $tarjetas, $customs, 'download');
        exit;
    }
    ...
    if ($action === 'print_pdf') {
        ...
        $precorte_data = pg_query_params($conn, "
            ...
            [$precorte_id]);
        $precorte = pg_fetch_assoc($precorte_data);
        $result = generatePrecortePDF($precorte_id, $terminal_id, $cashier_user_id, $from_ts, $to_ts, $precorte['opening_cash'], $system, $precorte['counted_cash'], $precorte['declared_credit'], $precorte['declared_debit'], $precorte['declared_custom'], $precorte['notes'], $tarjetas, $customs, 'print', true);
...
$open_tickets = pg_query_params($conn, "
    ...
    WHERE terminal_id = $1 AND user_id = $2 AND create_date >= $3 AND create_date < $4",
    [$terminal_id, $cashier_user_id, $from_ts, $to_ts]);
...
$voided_count = $alert_data['voided_tickets_cnt'] ?? 0;
$total_discounts = $alert_data['total_discounts'] ?? 0;
$threshold_alert = ($voided_count > 5 || $total_discounts > 100) ? "Alerta: $voided_count anulaciones (>5), $total_discounts MXN en descuentos (>$100)" : '';
?>
<!DOCTYPE html>
<html>
<head>
...
<body>
                    ...
                    <td>
                        <?php if (!$ticket['is_zero']): ?>
                            <form method="POST">
                                ...
                                <label>Terminal Bancaria (si aplica): <input type="text" name="card_reader"></label><br>
                                <label>Custom (si aplica): <input type="text" name="custom_name"></label><br>
                                <label>Monto: <input type="number" step="0.01" name="amount" value="<?php echo $ticket['total_amount']; ?>" required></label><br>
                                <label>Razón (si anular):
    ...
    <?php else: ?>
        ...
        <p>Efectivo: <?php echo $system['sys_cash'] ?? 0; ?> MXN</p>
        <p>Crédito: <?php echo $system['sys_credit'] ?? 0; ?> MXN</p>
        <p>Débito: <?php echo $system['sys_debit'] ?? 0; ?> MXN</p>
        <p>Custom: <?php echo $system['sys_custom_total'] ?? 0; ?> MXN</p>
        <p>Payouts: <?php echo $system['sys_payouts'] ?? 0; ?> MXN</p>
        ...
        <form method="POST" oninput="calculateTotal()">
            ...
            <input type="hidden" name="from_ts" value="<?php echo $from_ts; ?>">
            <input type="hidden" name="to_ts" value="<?php echo $to_ts; ?>">
            <input type="hidden" name="action" value="save_precorte">
            <label>Fondo Inicial: <input type="number" step="0.01" name="opening_cash" value="<?php echo $opening_cash; ?>" required></label><br>
            <label>Conteo Efectivo:
                <table>
                    <tr><th>Denominación</th><th>Cantidad</th><th>Subtotal</th></tr>
                    <?php foreach ([1000, 500, 200, 100, 50, 20, 10, 5, 2, 1, 0.5, 0.2, 0.1] as $denom): ?>
                        <tr>
                            <td><?php echo $denom; ?> MXN</td>
                            <td><input type="number" name="denoms[<?php echo $denom; ?>]" min="0" value="0" oninput="calculateTotal()"></td>
                            <td id="subtotal_<?php echo $denom; ?>">0.00</td>
                        </tr>
                    <?php endforeach; ?>
                </table>
            </label><br>
            <label>Total Efectivo: <input type="number" step="0.01" id="counted_cash" name="counted_cash" readonly></label><br>
            <p id="cash_diff">Diferencia: 0.00 MXN</p>
            <label>Crédito Declarado: <input type="number" step="0.01" name="declared_credit" value="0" required></label><br>
            <label>Débito Declarado: <input type="number" step="0.01" name="declared_debit" value="0" required></label><br>
            <label>Custom Declarado: <input type="number" step="0.01" name="declared_custom" value="0" required></label><br>
            <label>Notas: <textarea name="notes"></textarea></label><br>
            <button type="submit">Guardar e Imprimir Precorte</button>
        </form>

        <?php if (isset($_GET['precorte_id'])): ?>
            <h2>Opciones de Precorte</h2>
            <form method="POST">
                <input type="hidden" name="action" value="download_pdf">
                <input type="hidden" name="precorte_id" value="<?php echo $_GET['precorte_id']; ?>">
